...
spec = importlib.util.spec_from_file_location(
    "export_source", Path(__file__).resolve().parents[1] / "export_source.py"
)
    ...
    def test_export_preserves_source_and_excludes_private_artifacts(self):
        with tempfile.TemporaryDirectory() as directory:
            ...
            fixtures = {
                ...
                "frontend/.env.example": "TOKEN=placeholder\n",
                "backend/aciso_agent/agent.py": "root_agent = None\n",
                "frontend/.env.local": "PRIVATE=yes\n",
                "backend/archive/credentials.json": "private archive",
            ...
            self.assertEqual(
                ...
                {"README.md", "frontend/.env.example", "backend/aciso_agent/agent.py"},
            )
            manifest = json.loads((destination / "docs/source-manifest.json").read_text())
            self.assertEqual(manifest, result)
            for entry in manifest["files"]:
                self.assertEqual(
                    hashlib.sha256((destination / entry["path"]).read_bytes()).hexdigest(),
                    entry["sha256"],
                )
            for name, text in fixtures.items():
