...
// --- Sign In with Google (Popup version - may have CORS issues) ---
...
// --- Sign In with Google (Redirect version - CORS-safe) ---
  ...
  try {
    ...
    if (error instanceof FirebaseError) {
      throw new Error(mapAuthCodeToMessage(error.code));
        ...
        throw new Error('An unknown error occurred during Google redirect.');
    }
  }
};
...
export const handleGoogleRedirectResult = async (): Promise<User | null> => {
  try {
    ...
    if (result) {
      const user = result.user;
      console.log('User signed in with Google redirect:', user?.displayName);
      return user;
    }
    return null;
  } catch (error: unknown) {
    ...
    if (error instanceof FirebaseError) {
      throw new Error(mapAuthCodeToMessage(error.code));
        ...
        throw new Error('An unknown error occurred during Google redirect result handling.');
    }
  }
};
