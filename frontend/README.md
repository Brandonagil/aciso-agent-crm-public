...
Next.js dashboard, member-risk views and agent chat. The frontend uses React, TypeScript and Firebase, and proxies agent requests to the Python backend.
...
| `components/` | Dashboard, chat, reports and retention-plan views |
| `contexts/` | Authentication and application context |
| `lib/` | Cloud configuration, data access, types and shared utilities |
| `tests/unit/` | Offline component, fixture and timestamp checks |
| `tests/e2e/` | Browser scenarios for a configured application |
| `tests/fixtures/` | Example inputs for retention-plan rendering |
